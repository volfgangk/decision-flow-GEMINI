    ...
    if (depth > 1 && !isWinner && decision.voters > 0) {
      return (
        <div key={node.id} className="flex flex-col relative ml-7 mt-2 mb-1.5">
          ...
          {!isLastChild && <div className={`absolute left-[-22px] top-[14px] bottom-[-16px] border-l-2 ${parentIsWinner ? 'border-[#E8668A]' : 'border-[#D2DFEE] opacity-60'} -z-10`} />}
      ...
      <div key={node.id} className={`flex flex-col relative ${depth > 1 ? 'ml-7 mt-3' : 'mb-6'}`}>
        ...
        >
          <div className="flex items-center gap-3 flex-1 pr-2">
            ...
            <span className={`text-[14px] font-bold leading-snug break-keep ${isWinner ? 'text-gray-900' : 'text-gray-500'}`}>{node.text}</span>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {depth === 1 && <span className={`text-[12px] font-black ${isWinner ? 'text-[#E8668A]' : 'text-gray-400'}`}>{percent}%</span>}
            {children.length > 0 && (
              <button onClick={e => toggleExpand(node.id, e)} className={`w-7 h-7 rounded-full flex items-center justify-center border transition-all ${isExpanded ? isWinner ? 'bg-[#E8668A] border-[#E8668A] text-white rotate-180' : 'bg-gray-200 border-gray-200 text-gray-500 rotate-180' : 'bg-white border-gray-200 text-gray-400'}`}>
